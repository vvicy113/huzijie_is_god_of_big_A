 ...
 * 涨停板选择器。
 * <p>
 * 保留当日涨幅 &ge; minPct% 的股票（默认 9.5%，接近 A 股主板 10% 涨停限制）。
 */
public class LimitUpSelector implements StockSelector {

    private final double minPct;
    private final int code;

    public LimitUpSelector(double minPct, int code) {
    ...
    public List<String> select(List<String> candidates, LocalDate date) {
        ...
        String sql = """
                ...
                SELECT stock_code FROM chg
                WHERE prev_close IS NOT NULL AND prev_close > 0
                  AND (close - prev_close) / prev_close * 100 >= ?
                """.formatted(ph);
        ...
        try (Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            throw new RuntimeException("涨停筛选查询失败", e);
        }
        return result;
    }
