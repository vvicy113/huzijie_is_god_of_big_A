 ...
 * 成交量阈值选择器。
 * <p>
 * 保留当日成交量 &ge; N日均量 &times; 倍数 的股票。单条 SQL 批量查询。
 */
public class VolumeThresholdSelector implements StockSelector {

    private final int nDays;
    private final double multiplier;
    private final int code;

    public VolumeThresholdSelector(int nDays, double multiplier, int code) {
    ...
    public List<String> select(List<String> candidates, LocalDate date) {
        ...
        try (Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            throw new RuntimeException("成交量筛选查询失败", e);
        }
        return result;
    }
